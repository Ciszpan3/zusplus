...
import { User, Sun, CloudSun, Cloud, CloudRain, CloudLightning, TrendingUp, Plane, Briefcase, Umbrella, Info, GraduationCap } from "lucide-react";
...
const Results: React.FC = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    while (remainingHeight > 0) {
      ...
      if (ctx) {
        ctx.drawImage(canvas, 0, sourceYPosition, canvas.width, sourceHeight, 0, 0, canvas.width, sourceHeight);
        const pageImgData = pageCanvas.toDataURL("image/png", 1.0);
        if (sourceYPosition > 0) {
          pdf.addPage();
        }
        pdf.addImage(pageImgData, "PNG", padding, padding, imgWidth, pageHeight);
      }
  ...
  const DEFAULT_RETIREMENT_AGE = formData?.retirementYear ? parseInt(formData.retirementYear) - currentYear + DEFAULT_AGE : 67;
  ...
  const [projectedPensionNominal, setProjectedPensionNominal] = useState<number | null>(null);
  const [projectedPensionReal, setProjectedPensionReal] = useState<number | null>(null);

  // State for projected chart bar heights
  const [projectedBarHeightNominal, setProjectedBarHeightNominal] = useState<number>(120);
  const [projectedBarHeightReal, setProjectedBarHeightReal] = useState<number>(80);

  // State for calculated chart bar heights (right chart)
  const [calculatedBarHeightNominal, setCalculatedBarHeightNominal] = useState<number>(120);
  const [calculatedBarHeightReal, setCalculatedBarHeightReal] = useState<number>(120);
  ...
  const futurePensionReal = projectedPensionReal || prognosisData?.przyszla_emerytura_realna || 4890;
  const futurePensionNominal = projectedPensionNominal || prognosisData?.przyszla_emerytura_nominalna || 4890;
  ...
  const calculateBarHeight = (value: number, referenceValue: number, baseHeight: number) => {
    ...
    return Math.max(MIN_CHART_HEIGHT, Math.min(calculatedHeight, MAX_CHART_HEIGHT));
  };
  ...
  const handleReset = () => {
    ...
    if (prognosisData) {
      ...
      if (nominalHeight === MIN_CHART_HEIGHT && realHeight === MIN_CHART_HEIGHT) {
  ...
  }, [apiPensionNominal, apiPensionReal, prognosisData?.przyszla_emerytura_realna]);
  ...
  return <>
      <Header showDownloadButton={true} showBackButton={true} onDownloadPDF={handleDownloadPDF} />
      <div className="bg-gray-50 min-h-screen" ref={contentRef}>
        <div>
          ...
          <section className="bg-gradient-to-r from-[hsl(var(--green-primary))] to-[hsl(var(--blue-primary))] py-12 px-6">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                ...
                {/* Weather Card */}
                <div className="bg-white rounded-2xl p-6 shadow-lg cursor-pointer hover:shadow-xl transition-shadow" onClick={() => setIsWeatherModalOpen(true)}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className={`p-3 rounded-lg mt-4 ${percentDifference >= 0 ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                      ...
                      <p className={`font-bold text-lg ${percentDifference >= 0 ? "text-[hsl(var(--success))]" : "text-destructive"}`}>
                        ...
                        {percentDifference >= 0 ? "wyższa od średniej!" : "niższa od średniej!"}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <section className="py-16 px-6 bg-white">
            <div className="max-w-7xl mx-auto">
              ...
              {ileLat && <div className="text-center mb-12">
                  <img src={new URL("@/assets/ile-lat-image.png", import.meta.url).href} alt="Ile lat informacja" className="mx-auto mb-4 w-48 h-auto" />
              ...
              <div className="mb-12">
                ...
                <div className="grid md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                  <Button onClick={() => setIsRetirementModalOpen(true)} variant="outline" className="h-auto py-4 px-6 flex flex-col items-center gap-2 hover:bg-primary/5 border-2 hover:border-primary transition-all">
                  ...
                  <Button onClick={() => setIsInflationModalOpen(true)} variant="outline" className="h-auto py-4 px-6 flex flex-col items-center gap-2 hover:bg-primary/5 border-2 hover:border-primary transition-all">
                    <div className="bg-primary/10 p-3 rounded-full">
                  ...
                  <Button onClick={() => setIsPensionFundingModalOpen(true)} variant="outline" className="h-auto py-4 px-6 flex flex-col items-center gap-2 hover:bg-primary/5 border-2 hover:border-primary transition-all">
                    <div className="bg-primary/10 p-3 rounded-full">
              ...
              <div className="text-center mb-12">
                <div className="flex items-center justify-center gap-4 mb-3">
                  ...
                  <AIRecommendations retirementData={{
                  age,
                  gender,
                  retirementAge,
                  monthlyIncome,
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-xl p-8 shadow-lg">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <Slider value={[age]} onValueChange={val => setAge(val[0])} min={18} max={60} step={1} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-4">
                        <button onClick={() => setGender("female")} className={`flex-1 py-2 px-4 rounded-lg border-2 font-medium transition-colors ${gender === "female" ? "bg-[hsl(var(--success))] text-white border-[hsl(var(--success))]" : "bg-white text-gray-700 border-gray-300 hover:border-[hsl(var(--success))]"}`}>
                          Kobieta
                        </button>
                        <button onClick={() => setGender("male")} className={`flex-1 py-2 px-4 rounded-lg border-2 font-medium transition-colors ${gender === "male" ? "bg-gray-600 text-white border-gray-600" : "bg-white text-gray-700 border-gray-300 hover:border-gray-600"}`}>
                          Mężczyzna
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[retirementAge]} onValueChange={val => setRetirementAge(val[0])} min={60} max={70} step={1} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[monthlyIncome]} onValueChange={val => setMonthlyIncome(val[0])} min={4000} max={15000} step={100} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[careerBreaks]} onValueChange={val => setCareerBreaks(val[0])} min={0} max={10} step={1} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[sickLeaveDays]} onValueChange={val => setSickLeaveDays(val[0])} min={1} max={100} step={5} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[valorization]} onValueChange={val => setValorization(val[0])} min={0.1} max={20} step={0.1} className="w-full" />
                    </div>
                    ...
                    <div>
                      ...
                      <Slider value={[inflation]} onValueChange={val => setInflation(val[0])} min={-15} max={15} step={1} className="w-full" />
                    </div>

                    {/* Action Buttons */}
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      <button onClick={handleCalculate} className="bg-[hsl(var(--blue-primary))] text-white py-3 px-6 rounded-lg font-semibold hover:opacity-90 transition-opacity">
                        Aktualizuj
                      </button>
                      <button onClick={handleReset} className="bg-white border-2 border-[hsl(var(--warning))] text-[hsl(var(--warning))] py-3 px-6 rounded-lg font-semibold hover:bg-orange-50 transition-colors">
                        Reset
                      </button>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-6">
                    ...
                    <p className="text-[hsl(var(--blue-primary))] text-4xl font-bold">
                      {apiPensionNominal ? Math.round(apiPensionNominal).toLocaleString("pl-PL") : "---"}{" "}
                      PLN
                    </p>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-lg overflow-visible flex-1 flex flex-col justify-end">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-6 align-bottom">
                      {/* Projected Chart */}
                      <div className="text-center">
                        <div className="relative flex flex-col justify-end" style={{
                        height: "256px",
                        maxHeight: "256px"
                      }}>
                          {/* Bars */}
                          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-24">
                            <div className="relative bg-[#00993F] rounded-t-lg" style={{
                            height: `${projectedBarHeightNominal}px`
                              ...
                              <div className="absolute top-2 -right-20 sm:-right-24 flex items-center gap-1 sm:gap-2 min-w-max">
                                ...
                                <span className="text-[#00993F] font-bold text-[10px] sm:text-xs whitespace-nowrap">
                                  {Math.round(futurePensionNominal).toLocaleString()}{" "}
                                  PLN
                                </span>
                              </div>
                            </div>
                            <div className="relative bg-red-400 rounded-b-lg" style={{
                      ...
                      <div className="text-center">
                        ...
                        maxHeight: "256px"
                          ...
                          {/* Bars */}
                          {apiPensionNominal && apiPensionReal ? <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-24">
                              <div className="relative bg-[#00993F] rounded-t-lg" style={{
          ...
          {prognosisData?.szczegoly && <section className="py-16 px-6 bg-white">
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12">
                  <h2 className="text-gray-900 text-3xl font-bold mb-3">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--blue-primary))]/10 to-[hsl(var(--blue-primary))]/5 rounded-xl p-6 border-2 border-[hsl(var(--blue-primary))]/30 shadow-sm hover:shadow-md transition-all hover:border-[hsl(var(--blue-primary))]">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[hsl(var(--blue-primary))] text-3xl font-bold">
                        {prognosisData.szczegoly.podstawa_obliczenia_emerytury.toLocaleString("pl-PL", {
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--green-primary))]/10 to-[hsl(var(--green-primary))]/5 rounded-xl p-6 border-2 border-[hsl(var(--green-primary))]/30 shadow-sm hover:shadow-md transition-all hover:border-[hsl(var(--green-primary))]">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[hsl(var(--green-primary))] text-3xl font-bold">
                        {Math.round(prognosisData.szczegoly.srednie_dalsze_trwanie_zycia_miesiace / 12)}{" "}
                        lat
                      </p>
                      <p className="text-gray-500 text-xs">
                        (
                        {prognosisData.szczegoly.srednie_dalsze_trwanie_zycia_miesiace}{" "}
                        miesięcy)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--blue-primary))]/10 to-[hsl(var(--blue-primary))]/5 rounded-xl p-6 border-2 border-[hsl(var(--blue-primary))]/30 shadow-sm hover:shadow-md transition-all hover:border-[hsl(var(--blue-primary))]">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[hsl(var(--blue-primary))] text-3xl font-bold">
                        {prognosisData.szczegoly.szacowana_suma_skladek.toLocaleString("pl-PL", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2
                    })}{" "}
                        PLN
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--green-primary))]/10 to-[hsl(var(--green-primary))]/5 rounded-xl p-6 border-2 border-[hsl(var(--green-primary))]/30 shadow-sm hover:shadow-md transition-all hover:border-[hsl(var(--green-primary))]">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[hsl(var(--green-primary))] text-3xl font-bold">
                        {prognosisData.szczegoly.kapital_poczatkowy.toLocaleString("pl-PL", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2
                    })}{" "}
                        PLN
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-[hsl(var(--blue-primary))] text-2xl font-bold">
                          {((prognosisData.szczegoly.wspolczynnik_waloryzacji - 1) * 100).toFixed(2)}
                          %
                        </p>
                      </div>
                      <div className="border-t border-[hsl(var(--blue-primary))]/20 pt-2">
                        ...
                        <p className="text-[hsl(var(--blue-primary))] text-2xl font-bold">
                          {(prognosisData.szczegoly.wspolczynnik_inflacji * 100).toFixed(2)}
                          %
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--green-primary))]/10 to-[hsl(var(--green-primary))]/5 rounded-xl p-6 border-2 border-[hsl(var(--green-primary))]/30 shadow-sm hover:shadow-md transition-all hover:border-[hsl(var(--green-primary))]">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="border-t border-[hsl(var(--green-primary))]/20 pt-2">
                        ...
                        <p className="text-[hsl(var(--green-primary))] text-2xl font-bold">
                          {prognosisData.szczegoly.srednia_skladka_miesieczna.toLocaleString("pl-PL", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}{" "}
                          PLN
        ...
        <DashboardAIChat userEmail="" retirementData={{
        age,
        gender,
        retirementAge,
        monthlyIncome,
        ...
        {/* Educational Modals */}
        <RetirementCalculationModal isOpen={isRetirementModalOpen} onClose={() => setIsRetirementModalOpen(false)} />
        <InflationModal isOpen={isInflationModalOpen} onClose={() => setIsInflationModalOpen(false)} />
        <PensionFundingModal isOpen={isPensionFundingModalOpen} onClose={() => setIsPensionFundingModalOpen(false)} />
        <WeatherExplanationModal isOpen={isWeatherModalOpen} onClose={() => setIsWeatherModalOpen(false)} />
      </div>
    </>;
};
